...
# parser.add_argument('--dataset', type=str, default="cifar10", help="choose from cifar10,svhn")
parser.add_argument('--dataset', type=str, default="my_dataset", help="choose from cifar10,svhn")
...
parser.add_argument('--out_dir', type=str, default='./FAT_results_small_dataset', help='dir of output')
...
if args.dataset == "my_dataset":
        # clean up cache for gpu
        torch.cuda.empty_cache()
        # train
        trainset = torch.utils.data.ConcatDataset([
        torch.load('./data/small-cifar-10-batches-py/data_batch_1'),
        torch.load('ml_proj/Friendly-Adversarial-Training/data/small-cifar-10-batches-py/data_batch_2'),
        torch.load('ml_proj/Friendly-Adversarial-Training/data/small-cifar-10-batches-py/data_batch_3'),
        torch.load('ml_proj/Friendly-Adversarial-Training/data/small-cifar-10-batches-py/data_batch_4'),
        torch.load('ml_proj/Friendly-Adversarial-Training/data/small-cifar-10-batches-py/data_batch_5')
        ])
        train_loader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, num_workers=2)
        # test
        testset = torch.load('ml_proj/Friendly-Adversarial-Training/data/small-cifar-10-batches-py/test_batch')
        test_loader = torch.utils.data.DataLoader(testset, batch_size=128, shuffle=False, num_workers=2)
